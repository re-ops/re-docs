...
| host
| KVM node host
| link:kvm_localhost[KVM localhost]
...
* Deploying Re-core within as a VM that we would like to manage isn't possible due to link:https://wiki.libvirt.org/page/TroubleshootMacvtapHostFail[networking limitations] imposed by KVM (install Re-core outside any of the managed VMs).

* KVM uses the SSH binary, this means that .ssh/config have an effect on its connectivity, for example controlmaster (connections re-use) is discouraged it will cause connections issues, similarly known_hosts file requires us to contain each node key within it.
