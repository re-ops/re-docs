...
* KVM uses the SSH binary, this means that .ssh/config have an effect on its connectivity, for example controlmaster (connections re-use) is discouraged it will cause connections issues.
